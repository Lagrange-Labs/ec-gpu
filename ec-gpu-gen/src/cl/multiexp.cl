...
/*
 * ============================================================================
 * Multi-poly batch commit kernels
 * ============================================================================
 * These kernels process N same-size polynomials in a single set of kernel
 * launches. All polys share the same SRS bases. Their data is packed
 * contiguously, and bucket indices are offset by poly_idx to keep polys
 * separate in the sort space.
 *
 * Existing kernels that work unchanged with N*n_per_poly elements:
 *   to_scalar_bytes, preprocess_signed_digits, count_buckets,
 *   scatter_to_sorted, reduce_buckets_chunked, combine_chunks_to_windows
 *
 * New kernels below handle the poly-index-aware steps.
 */

/*
 * Multi-poly decompose: signed digits → (bucket_key, base_index) pairs.
 *
 * Same as POINT_decompose_to_pairs but offsets keys by
 * poly_idx * total_buckets_per_poly so each poly's buckets occupy a
 * separate range. Values store the local base index (within-poly),
 * which is correct since all polys share the same base buffer.
 *
 * Launch with N * n_per_poly * num_windows threads.
 */
KERNEL void POINT_decompose_to_pairs_multi(
    GLOBAL ushort *digits,
    GLOBAL uint *keys,
    GLOBAL uint *values,
    uint n_per_poly,
    uint num_windows,
    uint buckets_per_window,
    uint total_buckets_per_poly,
    uint total_pairs) {
  const uint gid = GET_GLOBAL_ID();
  if (gid >= total_pairs) return;
  const uint total_pairs_per_poly = n_per_poly * num_windows;
  const uint poly_idx = gid / total_pairs_per_poly;
  const uint local_pair = gid % total_pairs_per_poly;
  const uint local_base = local_pair / num_windows;
  const uint window = local_pair % num_windows;

  ushort raw = digits[gid];
  uint ind = raw & 0x7FFF;
  if (ind == 0) {
    keys[gid] = 0xFFFFFFFF;
    values[gid] = 0;
    ...
    uint sign = (raw >> 15) & 1;
    keys[gid] = poly_idx * total_buckets_per_poly + window * buckets_per_window + (ind - 1);
    values[gid] = local_base | (sign << 31);
  }
}

/*
 * Multi-poly prefix sum: N parallel prefix sums (1 thread per poly).
 *
 * Each thread scans its own bucket range and computes exclusive prefix sums.
 * The offset for each poly's sorted region starts at
 * poly_idx * total_pairs_per_poly.
 *
 * Launch with N threads (one per poly).
 */
KERNEL void POINT_prefix_sum_multi(
    GLOBAL uint *counts,
    GLOBAL uint *offsets,
    uint total_buckets_per_poly,
    uint total_pairs_per_poly,
    uint n_polys) {
  const uint poly_idx = GET_GLOBAL_ID();
  if (poly_idx >= n_polys) return;

  uint base_bucket = poly_idx * total_buckets_per_poly;
  uint running = poly_idx * total_pairs_per_poly;

  for (uint i = 0; i < total_buckets_per_poly; i++) {
    offsets[base_bucket + i] = running;
    running += counts[base_bucket + i];
  }
}

/*
 * Multi-poly bucket accumulation: 1 thread per bucket across all N polys.
 *
 * Empty buckets write POINT_ZERO and return early. This avoids the
 * nonempty_ids indirection and CPU-side download/dispatch complexity.
 * For medium polys (ws=2-4), bucket sizes are small, so 1-thread-per-bucket
 * is efficient.
 *
 * All polys share the same base buffer — values store local_base_idx
 * (within-poly index), so bases[local_base_idx] is correct.
 *
 * Launch with N * total_buckets_per_poly threads.
 */
KERNEL void POINT_accumulate_all_buckets_multi(
    GLOBAL POINT_affine *bases,
    GLOBAL uint *sorted_values,
    GLOBAL uint *bucket_offsets,
    GLOBAL uint *bucket_sizes,
...
/*
 * Multi-poly Horner reduction: 1 thread per poly.
 *
 * Each thread reduces its poly's window_results into one final commitment
 * using Horner's method (MSB-first).
 *
 * Launch with N threads (one per poly).
 */
KERNEL void POINT_reduce_windows_multi(
    GLOBAL POINT_jacobian *window_results,
  ...
  POINT_jacobian acc = POINT_ZERO;
  uint base = poly_idx * num_windows;
  for (int i = (int)num_windows - 1; i >= 0; i--) {
    uint w = window_size;
    uint remaining = effective_bits - (uint)i * window_size;
    if (w > remaining) w = remaining;
    ...
    acc = POINT_add(acc, window_results[base + i]);
  }
  final_results[poly_idx] = acc;
}
