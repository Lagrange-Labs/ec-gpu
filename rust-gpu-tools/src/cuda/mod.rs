...
impl Program {
    ...
    pub fn create_kernel(&self, name: &str, gws: usize, lws: usize) -> GPUResult<Kernel<'_>> {
    ...
    /// Synchronize the GPU stream, ensuring all previously launched kernels have completed.
    ///
    /// This must be called before reading results from GPU buffers when using `run_async()`.
    /// Note: `read_into_buffer()` already calls synchronize internally, so explicit calls
    /// are only needed when you want to ensure completion without reading data.
...
impl<'a> Kernel<'a> {
    ...
    ///
    /// Unlike `run()`, this does NOT call `stream.synchronize()` after launch.
    /// The caller must ensure synchronization happens before reading results
    /// (e.g., via `Program::synchronize()` or `read_into_buffer` which syncs internally).
    ///
    /// This enables kernel pipelining — the GPU can begin executing the next kernel
    /// while the previous one is still running, as long as there are no data dependencies.
    ///
    /// ### Panics
    ///
    /// Panics if the wrong number of arguments was provided.
    pub fn run_async(self) -> GPUResult<()> {
        ...
        // NO stream.synchronize() — caller is responsible for syncing at data-dependency points
        Ok(())
    }
}
